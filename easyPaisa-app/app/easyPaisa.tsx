import { useFonts } from "expo-font";
import { Stack } from "expo-router";
import * as SplashScreen from "expo-splash-screen";
import { StatusBar } from "expo-status-bar";
import { useEffect } from "react";
import "react-native-reanimated";
import { useColorScheme } from "@/hooks/useColorScheme";
import {
  Text,
  View,
  StyleSheet,
  Image,
  TouchableOpacity,
  SafeAreaView,
} from "react-native";
import MaterialIcons from "@expo/vector-icons/MaterialIcons";
// Prevent the splash screen from auto-hiding before asset loading is complete.
// const Height_Screen=Dimensions.get('screen').height
SplashScreen.preventAutoHideAsync();
export default function RootLayout() {
  const colorScheme = useColorScheme();
  const [loaded] = useFonts({
    SpaceMono: require("../assets/fonts/SpaceMono-Regular.ttf"),
  });
  useEffect(() => {
    if (loaded) {
      SplashScreen.hideAsync();
    }
  }, [loaded]);

  if (!loaded) {
    return null;
  }
  return (
    <SafeAreaView style={styles.container}>
      {/* Header */}
      <View style={styles.header}>
        <MaterialIcons
          name="notifications-none"
          style={styles.icon}
          size={24}
          color="white"
        />
        <Text style={styles.text}>easypaisa</Text>
        <MaterialIcons
          name="search"
          style={styles.icon}
          size={24}
          color="white"
        />
      </View>
      {/* hero Section*/}
      <View style={styles.hero}>
        <View style={styles.heroInside}>
          <View>
            <Image
              source={{
                uri: "data:image/jpeg;base64,",
              }}
              style={styles.heroImg}
            />
            <Text style={{ fontSize: 12, marginHorizontal: 10 }}>
              Available Balance
            </Text>
            <Text
              style={{
                fontSize: 19,
                fontWeight: "bold",
                color: "black",
                marginHorizontal: 10,
                marginVertical: 4,
              }}
            >
              Rs. 30,000.00
            </Text>
            <Text
              style={{
                fontSize: 11,
                color: "black",
                marginTop: 2,
                marginHorizontal: 10,
              }}
            >
              Tap To Hide Balance
            </Text>
          </View>
          <View style={styles.rewards}>
            <View style={styles.heroRewards}>
              <MaterialIcons name="stars" size={20} color="orange" />
              <Text style={styles.heroRewardsText}>My Rewards</Text>
            </View>
            <View style={styles.mainBtn}>
              <TouchableOpacity style={styles.outlineBtn}>
                <Text style={{ textAlign: "center", fontSize: 13 }}>
                  Upgrade Account
                </Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.bgBtn}>
                <Text style={{ color: "#fff", textAlign: "center" }}>
                  Add Cash
                </Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
      </View>
      {/* Cards */}
      <View style={styles.cards}>
        <View style={styles.designCard}>
          <MaterialIcons name="send" size={24} color="#14BB67" />
          <Text style={{ fontSize: 12, marginTop: 8 }}>Send Money</Text>
        </View>
        <View style={styles.designCard}>
          <MaterialIcons name="payments" size={24} color="#14BB67" />
          <Text style={{ fontSize: 12, marginTop: 8 }}>Bill Payments</Text>
        </View>
        <View style={styles.designCard}>
          <MaterialIcons name="mobile-screen-share" size={24} color="#14BB67" />
          <Text style={{ fontSize: 12, marginTop: 8 }}>Mobile Packages</Text>
        </View>
      </View>
      <View>
        <Text
          style={{
            fontSize: 15,
            marginVertical: 7,
            marginStart: 10,
            letterSpacing: -0.5,
            fontWeight: "500",
          }}
        >
          More with easypaisa
        </Text>
        <View style={styles.multiMarketingMainView}>
          <View style={styles.multipleCardsforMarketing}>
            <View style={styles.icon_TextMain}>
              <MaterialIcons name="install-mobile" size={24} color="#14BB67" />
              <Text style={{ fontSize: 12 }}>EasyLoad</Text>
            </View>
            <View style={styles.icon_TextMain}>
              <MaterialIcons name="monetization-on" size={24} color="#14BB67" />
              <Text style={{ fontSize: 12 }}>Easycash Loan</Text>
            </View>
            <View style={styles.icon_TextMain}>
              <MaterialIcons
                name="account-balance-wallet"
                size={24}
                color="#14BB67"
              />
              <Text style={{ fontSize: 12 }}>Saving Pockets</Text>
            </View>
            <View style={styles.icon_TextMain}>
              <MaterialIcons name="group-add" size={24} color="#14BB67" />
              <Text style={{ fontSize: 12 }}>Invite & Earn</Text>
            </View>
          </View>
          <View style={styles.multipleCardsforMarketing}>
            <View style={styles.icon_TextMain}>
              <MaterialIcons name="account-balance" size={24} color="#14BB67" />
              <Text style={{ fontSize: 12 }}>Balances</Text>
            </View>
            <View style={styles.icon_TextMain}>
              <MaterialIcons name="app-shortcut" size={24} color="#14BB67" />
              <Text style={{ fontSize: 12 }}>Mini App</Text>
            </View>
            <View style={styles.icon_TextMain}>
              <MaterialIcons name="credit-card" size={24} color="#14BB67" />
              <Text style={{ fontSize: 12 }}>{`Saving's`}</Text>
            </View>
            <View style={styles.icon_TextMain}>
              <MaterialIcons name="paypal" size={24} color="#14BB67" />
              <Text style={{ fontSize: 12 }}>Buy Now Pay</Text>
            </View>
          </View>
          <View style={styles.multipleCardsforMarketing}>
            <View style={styles.icon_TextMain}>
              <MaterialIcons name="translate" size={24} color="#14BB67" />
              <Text style={{ fontSize: 12 }}>Translate</Text>
            </View>
            <View style={styles.icon_TextMain}>
              <MaterialIcons
                name="volunteer-activism"
                size={30}
                color="#14BB67"
              />
              <Text style={{ fontSize: 12 }}>Donations</Text>
            </View>
            <View style={styles.icon_TextMain}>
              <MaterialIcons name="games" size={24} color="#14BB67" />
              <Text style={{ fontSize: 12 }}>Rs.1 Game</Text>
            </View>
            <View style={styles.icon_TextMain}>
              <MaterialIcons name="shape-line" size={24} color="#14BB67" />
              <Text style={{ fontSize: 12 }}>See All with</Text>
            </View>
          </View>
        </View>
      </View>
    </SafeAreaView>
  );
}
const styles = StyleSheet.create({
  container: {
    backgroundColor: "#F2F2F2",
  },
  text: {
    textAlign: "center",
    fontWeight: "600",
    color: "white",
    fontSize: 20,
    letterSpacing: 1,
  },
  header: {
    backgroundColor: "#249A54",
    height: 50,
    justifyContent: "space-between",
    flexDirection: "row",
    paddingVertical: 11,
    paddingHorizontal: 14,
  },
  icon: {
    fontWeight: "100",
  },
  hero: {
    backgroundColor: "#29B55F",
    height: 170,
  },
  heroInside: {
    backgroundColor: "#fff",
    borderRadius: 12,
    height: 150,
    margin: 10,
    justifyContent: "space-between",
    marginHorizontal: 10,
    flexDirection: "row",
  },
  heroImg: {
    width: 120,
    height: 50,
    marginHorizontal: 10,
    marginTop: 8,
  },
  heroRewards: {
    flexDirection: "row",
    margin: 22,
  },
  heroRewardsText: {
    fontWeight: "400",
  },
  rewards: {},
  mainBtn: {
    marginVertical: 5,
    gap: 6,
  },
  bgBtn: {
    backgroundColor: "#29B55F",
    borderRadius: 12,
    padding: 5,
    width: 125,
  },
  outlineBtn: {
    borderWidth: 1,
    borderColor: "#29B55F",
    borderRadius: 12,
    padding: 5,
    width: 125,
  },
  cards: {
    flexDirection: "row",
    marginTop: 20,
    justifyContent: "space-around",
  },
  designCard: {
    backgroundColor: "#fff",
    height: 90,
    width: 100,
    borderRadius: 10,
    justifyContent: "center",
    alignItems: "center",
    marginHorizontal: 4,
  },
  multiMarketingMainView: {
    backgroundColor: "#fff",
    height: 300,
  },
  icon_TextMain: {
    justifyContent: "center",
    alignItems: "center",
  },
  multipleCardsforMarketing: {
    flexDirection: "row",
    gap: 10,
    marginHorizontal: 8,
    marginVertical: 12,
    paddingHorizontal: 8,
    borderRadius: 10,
    justifyContent: "space-between",
  },
});
